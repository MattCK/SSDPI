...
class Campaign {
	...
	readonly _customerName: string;
	readonly _adShots: Set<AdShot>;
	readonly _powerPointFilename: string;
	readonly _powerPointURL: string;
	...
	*/
	constructor(campaignJSON: string) {

		//If the JSON is missing or not a string, throw an error
		if (campaignJSON == null) {throw "Campaign constructor: missing argument";}
		...
		if (baseCampaign == null) {throw "Campaign constructor: unable to parse JSON";}
		...
		for (let currentAdShotJSON of baseCampaign.adShots) {
			...
			if (newAdShot) {this._adShots.add(newAdShot);}
		}
	}
