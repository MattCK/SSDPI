...
class AdShot {
	...
	readonly _campaignID: number;
	readonly _requestedURL: string;
	readonly _storyFinder: boolean;
	readonly _mobile: boolean;
	readonly _belowTheFold: boolean;
	readonly _creatives: Set<Creative>;
	readonly _finalURL: string;
	readonly _pageTitle: string;
	readonly _injectedCreatives: Set<Creative>;
	...
	*/
	constructor(adShotJSON: string) {

		//If the JSON is missing or not a string, throw an error
		if (adShotJSON == null) {throw "AdShot constructor: missing argument";}
		...
		if (baseAdShot == null) {throw "AdShot constructor: unable to parse JSON";}
		...
		for (let injectedCreativeID of baseAdShot.injectedCreatives) {
			...
			for (let currentCreative of this._creatives) {
				if (currentCreative.id() == injectedCreativeID) {this._injectedCreatives.add(currentCreative);}
			}
		}
	}
