...
(function (asr) {
    ...
    */
    function getMenu() {
        ...
        let domainInputElement = base.nodeFromID("domain");
    ...
    let _matchingOrders = {}; //Retrieved DFP orders matching the latest search term
    let _lineItems = {}; //Array of line item names and their descriptions
    let _dfpCreatives = {}; //Array of creative IDs and their content
    let _creatives = new Map(); //Map of Creative IDs and their Creatives available to inject into pages
    function showTagCountAdded(numberOfTags) {
    ...
    */
    function getTagImages(tagTextArray) {
    ...
    */
    ...
    asr.uploadTagImage = uploadTagImage;
    function getCreative(serverResponse) {
    ...
    asr.getCreative = getCreative;
    function verifyCampaignFormData() {
        //Verify a customer name was entered
        let customerNameInput = base.nodeFromID("customerName");
    ...
    */
    function requestScreenshots() {
        ...
        //Make the request
        base.asyncRequest(_requestScreenshotsURL, { adShots: adShots,
            creativePriorities: creativePriorities,
            customerName: base.nodeFromID("customerName").value,
            backgroundID: base.nodeFromID("backgroundID").value }, onSuccessCallback, onFailCallback);
    }
    asr.requestScreenshots = requestScreenshots;
    /**
    ...
    */
                ...
                for (let orderID in _matchingOrders) {
                    if (_matchingOrders.hasOwnProperty(orderID)) {
                        ...
                        if (_matchingOrders[orderID].advertiserName) {
                            ...
                            if (_matchingOrders[orderID].agencyName) {
                                orderName += " (" + _matchingOrders[orderID].agencyName + ")";
                            }
                        }
                        orderName += " - " + orderID;
    ...
    asr.displayOrderNotes = displayOrderNotes;
        ...
        let onSuccessCallback = function (response) {
            ...
            if (response.success) {
                ...
                if (_matchingOrders[parseInt(orderID)].advertiserName) {
                    base.nodeFromID("customerName").value = _matchingOrders[parseInt(orderID)].advertiserName;
                }
            }
            else {
    ...
    asr.requestOrderData = requestOrderData;
    function useSelectedLineItems() {
        ...
        for (let index in lineItemIDs) {
            ...
            if (base.nodeFromID("lineItemsDiv").innerHTML != "") {
                base.nodeFromID("lineItemsDiv").innerHTML += "<br><br>";
            }
            ...
            for (let index in currentLineItem.creatives) {
                ...
                if (creativeIDList.indexOf(currentCreativeID) < 0) {
                    creativeIDList.push(currentCreativeID);
                }
                else {
                    console.log("Found: " + currentCreativeID);
                }
            }
        }
    ...
    asr.useSelectedLineItems = useSelectedLineItems;
    function selectAllLineItems() {
        $('#lineItemsTable input:checkbox').prop('checked', true);
        onLineItemSelection();
    }
    asr.selectAllLineItems = selectAllLineItems;
    function unselectAllLineItems() {
        $('#lineItemsTable input:checkbox').prop('checked', false);
        onLineItemSelection();
    }
    asr.unselectAllLineItems = unselectAllLineItems;
    function onLineItemSelection() {
        //Get the selected line item IDs
        let lineItemIDs = [];
        $("#lineItemsTable input:checked").each(function (index) {
            lineItemIDs.push($(this).val());
        ...
        if (creativeIDList.length >= 15) {
            base.show("tooManyCreativeDiv");
        }
        else {
            base.hide("tooManyCreativeDiv");
        }
    }
    asr.onLineItemSelection = onLineItemSelection;
    /**
    * Uploads the PowerPoint background image to the server where it is placed in official tag image storage
    */
    function uploadPowerPointBackground() {
        //Verify the fields were filled in
        if (base.nodeFromID("newBackgroundName").value.length == 0) {
            alert("Enter a name for the PowerPoint");
            return;
        }
        if (base.nodeFromID("newBackgroundImage").value.length == 0) {
            alert("Choose an image for the background");
            return;
        }
        ...
        let fileNode = base.nodeFromID("newBackgroundImage");
        let imageFile = fileNode.files[0];
        let formData = new FormData();
    ...
    asr.uploadPowerPointBackground = uploadPowerPointBackground;
    /**
    ...
    */
    function showErrorMessage(whileClause) {
        //If a "while" clause was passed, create the proper text
        let whileText = "";
        if (whileClause) {
            whileText = " while " + whileClause;
        }
    ...
    asr.showErrorMessage = showErrorMessage;
    /**
    ...
    */
    ...
    asr.enableSubmitButtons = enableSubmitButtons;
    function addTestPages() {
        ...
        let testSite = sites.values().next().value;
        let menuItemCount = testSite.menuItems().size;
        for (let menuIndex = 0; menuIndex < menuItemCount; ++menuIndex) {
            for (let pageIndex = 0; pageIndex < 4; ++pageIndex) {
                testSite.addSitePageRow();
            }
        }
        //Set the menu item and options for each page
        let allRowElements = document.querySelectorAll(".siteDiv tr");
        let rowIndex = 0;
        let menuIndex = 0;
        while (rowIndex < allRowElements.length) {
            //Set the first page simply to the menu item
            let currentRow = allRowElements[rowIndex];
            let currentPrefix = currentRow.id.substr(0, currentRow.id.length - 4);
            let rowMenu = document.getElementById(currentPrefix + "-URLInput");
            rowMenu.selectedIndex = menuIndex;
            ++rowIndex;
            //Set the second page to the menu item and top story
            currentRow = allRowElements[rowIndex];
            currentPrefix = currentRow.id.substr(0, currentRow.id.length - 4);
            rowMenu = document.getElementById(currentPrefix + "-URLInput");
            rowMenu.selectedIndex = menuIndex;
            let storyCheckbox = document.getElementById(currentPrefix + "-findStoryCheckbox");
            ...
            rowMenu = document.getElementById(currentPrefix + "-URLInput");
            rowMenu.selectedIndex = menuIndex;
            let mobileCheckbox = document.getElementById(currentPrefix + "-mobileCheckbox");
            ...
            rowMenu = document.getElementById(currentPrefix + "-URLInput");
            rowMenu.selectedIndex = menuIndex;
            storyCheckbox = document.getElementById(currentPrefix + "-findStoryCheckbox");
            storyCheckbox.checked = true;
            mobileCheckbox = document.getElementById(currentPrefix + "-mobileCheckbox");
            mobileCheckbox.checked = true;
            ++rowIndex;
            ++menuIndex;
        }
    }
    asr.addTestPages = addTestPages;
    $(function () {
