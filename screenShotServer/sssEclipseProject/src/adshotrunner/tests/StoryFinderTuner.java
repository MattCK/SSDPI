...
public class StoryFinderTuner {
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	public StoryFinderTuner(String url, int viewWidth, int viewHeight) throws MalformedURLException, URISyntaxException, UnsupportedEncodingException {
	...
	private String getURIDomain(String URIString) throws URISyntaxException, UnsupportedEncodingException, MalformedURLException {
		...
		if(URIString.contains("://") && ((URIString.contains("http") || URIString.contains("spdy"))) && (!URIString.startsWith("javascript"))){
			//System.out.println("uristringtotest: " + URIString);
			...
			//System.out.println("URIStr: " + URIString);
			//System.out.println("gethost: " + uriObject.getHost());
	...
	public class Scorer {		
		...
		/**
		 * Handicap anchor receives where the host is not the same as the target url (i.e. contests.newssite.com www.newssite.com). DEFAULT: -5
		 */
		private int DIFFERENTHOSTHANDICAP;
		...
		public Scorer() {
			...
			//Set same path and host score
			SAMEPATHPARTSCORE = 7;
			DIFFERENTHOSTHANDICAP = -8;
		 ...
		 * @throws MalformedURLException 
		 */
		public String getStory() throws MalformedURLException, UnsupportedEncodingException, URISyntaxException {
			...
			for (Map.Entry<Integer, Integer> currentScore : anchorScores.entrySet()) {	
				if(currentScore.getValue() > 5){
					System.out.println("n: "+ rankedClasses.get(0));
					System.out.println("h: " + anchors.get(currentScore.getKey()).get("class"));
					System.out.println("Score: " + currentScore.getValue());
					System.out.println("link: " + anchors.get(currentScore.getKey()).get("href"));
					System.out.println("------------------------------------------------------------");
				}
		 ...
		 * @throws MalformedURLException 
		 */
		public HashMap<Integer, Integer> getAnchorScores() throws MalformedURLException, UnsupportedEncodingException, URISyntaxException {
			...
			adjustScoresByHost(anchorScores);
			adjustScoreIfAllCaps(anchorScores);
			
			/*for (Map.Entry<Integer, Integer> currentScore : anchorScores.entrySet()) {	
				System.out.println("-------------------------------------");
				System.out.println("Anchor: " + anchors.get(currentScore.getKey()).get("href"));
				System.out.println("Score: " + currentScore.getValue());
				System.out.println(": ");
			}*/
		 ...
		 * @throws UnsupportedEncodingException 
		 */
		public void adjustScoresByHost(HashMap<Integer, Integer> anchorScores) throws MalformedURLException, UnsupportedEncodingException, URISyntaxException {
			
			//Grab the first path part of the target url for comparison
			String targetHost = getHostOfURI(targetURL);
			System.out.println("headURLHost: " + targetHost);
			...
			if (targetHost.length() > 1) {
				...
				for (Map.Entry<Integer, Integer> currentScore : anchorScores.entrySet()) {			    
					...
					//Grab the current anchor's href and path part
					String urlHref = anchors.get(currentScore.getKey()).get("href");
					String urlHost = getHostOfURI(urlHref);
					//String urlDomain = getURIDomain(urlHref);
					
					//System.out.println("Href: "  + urlHref);
					//System.out.println("Host: " + urlHost + " TargetHost: " + targetHost);
					
					//If the targetURL and current anchor path parts are the same, increment the score
					if ((!targetHost.equals(urlHost)) && (!urlHost.equals(""))) {
						System.out.println("Host: " + urlHost + " TargetHost: " + targetHost);
						scoreOffset += DIFFERENTHOSTHANDICAP;
						System.out.println("scoringDownforHost");
					}
		...
		/**
		 * Returns the first Host of a URI.
		 * 
		 * In other words, the part right before the domain part. For example, 'contest.boston.com/entertainment' would return
		 * 'contest'.
		 * 
		 * @param URIString		URI to get first path part
		 * @return				First path part (i.e. 'entertainment' of 'boston.com/entertainment' 
		 ...
		 * @throws UnsupportedEncodingException 
		 */
		private String getHostOfURI(String URIString) throws MalformedURLException, UnsupportedEncodingException {
			
			//System.out.println("gonnaDie: " + URIString);
			URIString = URIString.replace("\"", "");
			String hostName = "";

			if (URIString.contains("://") && ((URIString.contains("http") || URIString.contains("spdy"))) && (!URIString.startsWith("javascript"))){
				//URIString = URLEncoder.encode(URIString, "UTF-8");
				URL urler = new URL(URIString);
				hostName = urler.getHost();
			}
			return hostName;
		}
