...
public class CampaignRunner implements Runnable {
	...
	public void run() {
		...
		ArrayList<AdShotOriginal> adShotList = new ArrayList<AdShotOriginal>();
		for (Map<String, String> currentPage : requestInfo.pages) {
			...
			if (onlyScreenshot) {
				AdShotOriginal newAdShot = AdShotOriginal.create(pageURL);
				...
				for (TagImage singleTag: tagImages) {
					AdShotOriginal newAdShot = AdShotOriginal.create(pageURL, singleTag);
					if (!alternateURLs.isEmpty()) {newAdShot.addAlternatePageURL(alternateURLs);}
					newAdShot.useMobile(currentPage.get("useMobile").equals("1"));
					adShotList.add(newAdShot);
				}
				...
				AdShotOriginal newAdShot = AdShotOriginal.create(pageURL, tagImages);
				if (!alternateURLs.isEmpty()) {newAdShot.addAlternatePageURL(alternateURLs);}
				newAdShot.useMobile(currentPage.get("useMobile").equals("1"));
				adShotList.add(newAdShot);
			}
		...
		List<AdShotOriginal> standardAdShots = new ArrayList<AdShotOriginal>();
		List<AdShotOriginal> mobileAdShots = new ArrayList<AdShotOriginal>();
		for (AdShotOriginal currentAdShot : adShotList) {
		...
		try {
			...
			CampaignPowerPointOriginal powerPoint = new CampaignPowerPointOriginal(requestInfo.customer, titleDate, requestInfo.powerPointFontColor, backgroundFile);
			for (AdShotOriginal currentAdShot : adShotList) {
		...
		CampaignEmailOriginal resultsEmail = CampaignEmailOriginal.createCampaignEmail(
