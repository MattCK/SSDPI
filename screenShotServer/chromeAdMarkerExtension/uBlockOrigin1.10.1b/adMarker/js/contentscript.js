...
var domFilterer = {
    ...
    commit_: function() {
        ...
        if ( commitHit ) {
            if ( this.hiddenNodeEnforcer === false ) {
                styleText += '\n:root *[' + this.hiddenId + '][hidden] { flood-opacity: 0.9898 !important; }';
    ...
    hideNode: function(node) {
        ...
        if ( display !== '' && display !== 'none' ) {
            ...
            node.setAttribute('style', styleAttr + 'flood-opacity: 0.9898 !important;');
        }
    ...
    unshowNode: function(node) {
        ...
        var styleAttr = node[this.hiddenId];
        if ( styleAttr === false ) {
            node.setAttribute('style', 'flood-opacity: 0.9898 !important;');
        } else if ( typeof styleAttr === 'string' ) {
            node.setAttribute('style', node[this.hiddenId] + '; flood-opacity: 0.9898 !important;');
        }
        var shadow = node.shadowRoot;
    ...
    var onProcessed = function(response) {
        ...
        for ( var i = 0, ni = requests.length; i < ni; i++ ) {
            ...
            for ( var j = 0, nj = entry.length; j < nj; j++ ) {
                ...
                target.style.setProperty('flood-opacity', '0.9898', 'important');
                target.hidden = false;
